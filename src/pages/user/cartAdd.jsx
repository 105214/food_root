...
const AddToCart = () => {
  ...
  const handleBuyAllItems = () => {
    // Navigate to buy page with all cart items
    navigate('/user-buy', { state: { cart } });
  };
...
//         setCart(processedCart);
//         console.log("Processed cart state:", processedCart);
//         console.log("Full cart response:", response.data);
...
//       }
//     } catch (err) {
//       console.error("Detailed cart fetch error:", err);
//       setError(err.response?.data?.message || "Failed to load cart");
//     } finally {
//       setLoading(false);
//     }
...
//           <div className="cart-items-grid">
//             {cart.map((item) => (
//               <Card key={item.dishId || Math.random()} className="cart-item-card shadow mb-3">
//                 <Card.Body>
//                   <Card.Title>{item.name|| 'Unnamed Item'}</Card.Title>
//                   <Card.Text>Price: ₹{item.price}</Card.Text>
//                   <Card.Text>Quantity: {item.quantity || 0}</Card.Text>
//                   <div className="d-flex justify-content-between">
//                     <Button 
//                       variant="danger" 
//                       onClick={() => handleRemoveCart(item.dishId)}
